...
async def run(bot, message):
    ...
    # Handling multiple target channels
    if len(channels) > 1:
        for channel in channels:
            buttons.append([KeyboardButton(f"{channel['title']}")])
            btn_data[channel['title']] = channel['chat_id']
        buttons.append([KeyboardButton("cancel")]) 
        _toid = await bot.ask(message.chat.id, Translation.TO_MSG.format(_bot['name'], _bot['username']), 
    ...
    if fromid.text and not fromid.forward_date:
        ...
        await message.reply_text("Invalid Source!")
        return 
    ...
    await message.reply_text(
        text=Translation.DOUBLE_CHECK.format(botname=_bot['name'], botuname=_bot['username'], 
